...
router.post('/compra', async (req, res) => {
  ...
  try {
    ...
    await new sql.Request(transaction)
      ...
      .query(`INSERT INTO StockInsumos (producto_id, tipo, cantidad, costo_total, proveedor, observacion)
              VALUES (@producto_id, 'compra', @cantidad, @costo_total, @proveedor, @observacion)`);
    await new sql.Request(transaction)
      .input('producto_id', sql.Int, producto_id)
      .input('cantidad', sql.Decimal(10,2), cantidad)
...
router.post('/aplicacion', async (req, res) => {
  const { producto_id, cantidad, lote_id, observacion } = req.body;
  const pool = await getPool();
  const transaction = new sql.Transaction(pool);
  try {
    await transaction.begin();
    ...
    await new sql.Request(transaction)
      ...
      .input('observacion', sql.NVarChar, observacion || '')
      .query(`INSERT INTO StockInsumos (producto_id, tipo, cantidad, lote_id, observacion)
              VALUES (@producto_id, 'aplicacion', @cantidad, @lote_id, @observacion)`);
    await new sql.Request(transaction)
      .input('producto_id', sql.Int, producto_id)
      .input('cantidad', sql.Decimal(10,2), cantidad)
...
router.post('/egreso', async (req, res) => {
  ...
  try {
    ...
    await new sql.Request(transaction)
      ...
      .query(`INSERT INTO StockInsumos (producto_id, tipo, cantidad, lote_id, observacion)
              VALUES (@producto_id, @tipo, @cantidad, @lote_id, @observacion)`);
    await new sql.Request(transaction)
      .input('producto_id', sql.Int,          producto_id)
      .input('cantidad',    sql.Decimal(10,2), cantidad)
      .query('UPDATE Productos SET stock_actual = stock_actual - @cantidad WHERE id = @producto_id');
...
router.get('/historial/:producto_id', async (req, res) => {
  try {
    ...
    const result = await pool.request()
      ...
      .query(`SELECT s.tipo, s.cantidad, s.costo_total, s.fecha, s.proveedor, s.observacion,
              l.nombre AS lote
              FROM StockInsumos s
              LEFT JOIN Lotes l ON s.lote_id = l.id
