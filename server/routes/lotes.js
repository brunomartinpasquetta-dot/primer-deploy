...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await dbReq.query(
      `SELECT l.id, l.nombre, l.hectareas, l.variedad_id,
              vf.nombre AS variedad_nombre,
              vf.nombre AS variedad,
              l.cantidad_plantines,
...
router.post('/', async (req, res) => {
  try {
    const { nombre, hectareas, temporada_id, variedad_id, cantidad_plantines } = req.body;
    const pool = await getPool();
    await pool.request()
      ...
      .query(`INSERT INTO Parcelas (nombre, hectareas, temporada_id, variedad_id, cantidad_plantines)
              VALUES (@nombre, @hectareas, @temporada_id, @variedad_id, @cantidad_plantines)`);
    res.json({ ok: true });
  } catch (err) {
    console.error(err); res.status(500).json({ error: "Error interno del servidor" });
  }
...
router.put('/:id', async (req, res) => {
  try {
    const { nombre, hectareas, variedad_id, cantidad_plantines } = req.body;
    const pool = await getPool();
