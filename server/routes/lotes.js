...
router.post('/', async (req, res) => {
  try {
    ...
    await pool.request()
      ...
      .query(`INSERT INTO Lotes (nombre, hectareas, temporada_id, variedad, cantidad_plantines)
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .query(`UPDATE Lotes
              ...
              WHERE id=@id`);
    if (!result.recordset.length) return res.status(404).json({ error: 'Lote no encontrado' });
    res.json({ ok: true, lote: result.recordset[0] });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
