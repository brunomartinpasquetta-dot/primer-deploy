...
- PERMITIDO únicamente: lectura de datos (SELECT), lectura de archivos, análisis de código, generación de planes.
- Si el análisis de scope requiere verificar datos en DB, usar únicamente SELECT.
- Bash SOLO para comandos de lectura (git log, git status, git diff). PROHIBIDO git commit, git add, node, npm, kill, rm.
...
- CONTEXTO-PROYECTO-v7.txt
- REGLAS-DESARROLLO.md
- git log --oneline -5 (para saber el estado actual)
...
- Siempre verificar con git que el archivo a modificar no tiene cambios sin commitear
